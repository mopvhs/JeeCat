...
public class DingDanXiaApiService {
    ...
    private static List<String> specialSceneIdUrls = new ArrayList<>();
    private static Map<Integer, Integer> failRetrySceneIdMap = new HashMap<>();

    static {
        specialSceneIdUrls.add("https://3.cn/");
        specialSceneIdUrls.add("https://item.jd.com/");

        failRetrySceneIdMap.put(1, 2);
...
//        Result<String> urlPrivilege = getUrlPrivilege(apiKey, materialId, unionId, positionId);
     ...
     * @return
     */
    public Result<JdUnionIdPromotion> doGetJdByUnionidPromotion(String apiKey, String materialId, long unionId, long positionId, String couponUrl, Integer sceneId, boolean failRetry) {

        if (sceneId == null) {
            sceneId = getSceneId(materialId);
        }
        String url = "http://api.tbk.dingdanxia.com/jd/by_unionid_promotion?apikey=%s&materialId=%s&unionId=%d&positionId=%d&autoSearch=true&sceneId=%d";
        try {
            String encode = URLEncoder.encode(materialId);
            url = String.format(url, apiKey, encode, unionId, positionId, sceneId);
            ...
            try {
                ...
                if (failRetry && code == -1 && (msg.contains("sceneId未入参") || msg.contains("sceneId和商品格式不匹配") || msg.contains("数据返回失败【materialId链接无法识别】"))) {
                    // 判断是不是sceneId错误 {"code":-1,"msg":"数据返回失败【(sceneId未入参）或（sceneId和商品格式不匹配）】","data":[]}
                    Integer failBackSceneId = failRetrySceneIdMap.get(sceneId);
                    return doGetJdByUnionidPromotion(apiKey, materialId, unionId, positionId, couponUrl, failBackSceneId, false);
                }
                if (code != 200) {
    ...
    private int getSceneId(String materialId) {
        for (String url : specialSceneIdUrls) {
            if (materialId.contains(url)) {
                return 2;
            }
        }
        return 1;
    ...
    /**
     *
     * https://www.dingdanxia.com/doc/292/94
     * 通过unionId获取京东商品链接/活动链接/店铺链接【转链】【升级版】
     *
     * @param apiKey
     * @param materialId 推广物料url，例如活动链接、商品链接、联盟链接（包含微信shortlink形式）、联盟商品ID（itemId）、联盟商品链接（jingfen.jd.com/detail/{itemId}.html）等；单品转链时，sceneId=2时支持入参京东主站商品ID、京东商品链接（包含3.cn单品链接）；其他场景仅支持入参联盟商品ID、联盟商品链接。非单品转链不影响。
     * @param unionId
     * @param positionId
     * @return
     */
    public Result<String> getUrlPrivilege(String apiKey, String materialId, long unionId, long positionId) {
        String url = "http://api.tbk.dingdanxia.com/jd/url_privilege";
        ...
        try {
            ...
            try {
                ...
                log.error("转换异常! keyword:{}", materialId, e);
            }
        } catch (Exception e) {
            log.error("转换异常!! keyword:{}", materialId, e);
        }
